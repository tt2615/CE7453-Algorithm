...
class InputParser():
    ...
    def parse(self):
        ...
        with open(self.path, 'r') as f:
            ...
            while line != '':
                coor = line.split()
                coors.append(np.array([float(coor[0]), float(coor[1])]))
                line = f.readline()
...
class BSpline():
    ...
    def draw(self, path, x,y):
        ...
        # plt.plot(c_x,c_y,'k--',label='Control polygon',marker='s')
        plt.plot(p_x, p_y, 'o', label='Points', color='grey')
        plt.plot(x, y, '--', color='black', label='Original Curve')
        plt.plot(out[0],out[1],'r',linewidth=2.0,label='B-spline curve')
        plt.legend(loc='best')
...
class InterpolationAlgo():
    ...
    def __init__(self, input, degree=3, delta=1e-5):
        self.input = input
        self.degree = degree
        self.delta = delta
    ...
    def _get_knots(self, t_list, degree):
        ...
        for i in range(1, len(joint_list)):
            if joint_list[i] == prev_knot:
                joint_list[i] += self.delta * delta_count
                delta_count += 1
            else:
...
if __name__ == '__main__':
    ...
    b_spline.draw(f"{config.dir_path}/{config.output_fig_name}")